...
abstract class PollDetailModel with _$PollDetailModel {
  ...
  factory PollDetailModel.fromJson(Map<String, dynamic> json) => _$PollDetailModelFromJson(json);
}
...
extension PollDetailModelX on PollDetailModel {
  PollDetail toDomain() => PollDetail(
    ...
    dublicateResultEmployees: dublicateResultEmployees?.map((employee) => employee.toDomain()).toList(),
    allowedEmployees: allowedEmployees?.map((employee) => employee.toDomain()).toList(),
    deniedEmployees: deniedEmployees?.map((employee) => employee.toDomain()).toList(),
    allowedDepartments: allowedDepartments?.map((department) => department.toDomain()).toList(),
    deniedDepartments: deniedDepartments?.map((department) => department.toDomain()).toList(),
