...
class PollsListBloc extends Bloc<PollsListEvent, PollsListState> {
  ...
  PollsListBloc({required this.getPollsUsecase, required this.downloadFileUsecase})
  ...
  Future<void> _onLoadPolls(int? status, Emitter<PollsListState> emit) async {
    ...
    await result.fold((error) async => emit(PollsListState.error(error.message)), (polls) async {
  ...
  Future<void> _onRefreshPolls(int? status, Emitter<PollsListState> emit) async {
    final currentState = state.maybeWhen(loaded: (polls, coverImages) => (polls, coverImages), orElse: () => null);

    if (currentState != null) {
      // Keep showing current polls while refreshing
      emit(PollsListState.loaded(polls: currentState.$1, coverImages: currentState.$2));
    }
  ...
  Future<void> _loadCoverImages(List<dynamic> polls, Emitter<PollsListState> emit) async {
    final coverImages = <int, Uint8List>{};

    // Download images in parallel for polls that have cover
    final futures = polls.where((poll) => poll.cover != null && poll.cover!.isNotEmpty).map((poll) async {
      final result = await downloadFileUsecase(systemType: SystemType.kp, download: false, uriFile: poll.cover);
    ...
    state.maybeWhen(
      loaded: (polls, _) => emit(PollsListState.loaded(polls: polls, coverImages: coverImages)),
      orElse: () {},
    );
  }
}
