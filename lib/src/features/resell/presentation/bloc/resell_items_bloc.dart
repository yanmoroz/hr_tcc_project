...
class ResellItemsBloc extends Bloc<ResellItemsEvent, ResellItemsState> {
  ...
  ResellItemsBloc(this._getResellItemsUsecase) : super(const ResellItemsState.initial()) {
  ...
  Future<void> _onLoadResellItems(LoadResellItems event, Emitter<ResellItemsState> emit) async {
    emit(const ResellItemsState.loading());
    await _loadItems(emit, 0, 1); // Default status = 1 (Active)
  }

  Future<void> _onLoadMore(LoadMore event, Emitter<ResellItemsState> emit) async {
  ...
  Future<void> _onRefreshItems(RefreshItems event, Emitter<ResellItemsState> emit) async {
  ...
  Future<void> _onFilterByStatus(FilterByStatus event, Emitter<ResellItemsState> emit) async {
    emit(const ResellItemsState.loading());
  ...
  Future<void> _loadItems(
    ...
    final result = await _getResellItemsUsecase(status: status, page: page, pageSize: _pageSize);
    ...
    if (!emit.isDone) {
      result.fold(
        ...
        (items) {
          final allItems = existingItems != null ? [...existingItems, ...items] : items;
