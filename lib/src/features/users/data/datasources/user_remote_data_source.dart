...
abstract class UserRemoteDataSource {
  Future<Result<List<UserModel>>> getUsers({required SystemType systemType, String? search});
}
...
class UserRemoteDataSourceImpl implements UserRemoteDataSource {
  ...
  @override
  Future<Result<List<UserModel>>> getUsers({required SystemType systemType, String? search}) async {
    return ApiCallExecutor.executeApiCall(
      apiCall: () {
        final queryParameters = <String, String>{'systemType': systemType.value};
        if (search != null && search.isNotEmpty) {
          queryParameters['search'] = search;
        }
        return _apiClient.get(ApiConstants.usersEndpoint, queryParameters: queryParameters);
      },
