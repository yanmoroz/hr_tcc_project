...
void main() {
  group('StaffRemoteDataSource', () {
    ...
    group('getStaff', () {
      test(
        'should fetch staff items for EMPLOYEE target from API and map to models',
        () async {
          // Act
          final result = await dataSource.getStaff(
            target: StaffTarget.employee,
          );
      ...
      test(
        'should fetch staff items for DEPARTMENT target from API and map to models',
        () async {
          // Act
          final result = await dataSource.getStaff(
            target: StaffTarget.department,
          );
      ...
      test(
        'should fetch staff items for ORGANISATION target from API and map to models',
        () async {
          // Act
          final result = await dataSource.getStaff(
            target: StaffTarget.organisation,
          );
      ...
      test(
        'should fetch staff items with search parameter from API and map to models',
        () async {
          // Act
          final result = await dataSource.getStaff(
            target: StaffTarget.employee,
