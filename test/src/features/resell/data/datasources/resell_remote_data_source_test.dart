...
void main() {
  group('ResellRemoteDataSource', () {
    ...
    group('getResellItems', () {
      test(
        'should fetch list of resell items with status filter from API and map to models',
        () async {
          // Act
    ...
    test('should fetch list of resell items with search filter', () async {
      ...
      result.fold((error) => fail('Unexpected error: ${error.toString()}'), (
        response,
      ) {
        expect(response, isA<ResellListResponseModel>());
    ...
    group('getResellDetail', () {
      test(
        'should fetch resell item detail by id from API and map to model',
        () async {
          // First get a list to extract an ID
          final listResult = await dataSource.getResellItems(
            status: ResellStatus.onSale,
            page: 0,
            pageSize: 1,
          );
          ...
          await listResult.fold(
            (failure) => fail('Failed to get list: ${failure.message}'),
            (response) async {
    ...
    test('should initiate booking for a resell item', () async {
      ...
      await listResult.fold(
        (error) => fail('Failed to get list: ${error.toString()}'),
        (response) async {
          if (response.items.isEmpty) {
        ...
        (response) async {
          ...
          confirmResult.fold(
            (error) {
              ...
              AppLogger.d(
                'Confirmation failed (expected): ${error.toString()}',
              );
              expect(error, isA<Exception>());
            },
            (_) {
    ...
    test('should cancel booking', () async {
      ...
      await listResult.fold(
        ...
        (response) async {
          if (response.items.isEmpty) {
            AppLogger.d(
              'No booked items available to test cancel booking endpoint',
            );
            return;
          }
          ...
          cancelResult.fold(
            (error) {
              ...
              AppLogger.d(
                'Cancellation failed (expected): ${error.toString()}',
              );
              expect(error, isA<Exception>());
            },
            (_) {
