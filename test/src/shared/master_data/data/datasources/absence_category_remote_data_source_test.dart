...
void main() {
  group('AbsenceCategoryRemoteDataSource', () {
    late AbsenceCategoryRemoteDataSource dataSource;
    ...
    setUp(() {
      ...
      dataSource = AbsenceCategoryRemoteDataSourceImpl(apiClient);
    });

    group('getAbsenceCategories', () {
      test('should fetch absence categories from API and map to models', () async {
        // Act
        final result = await dataSource.getAbsenceCategories();
        ...
        result.fold(
          ...
          (models) {
            ...
            expect(models, isA<List<AbsenceCategoryModel>>());
            expect(models.isNotEmpty, isTrue);

            // Print the actual data for verification
            print('Fetched ${models.length} absence categories:');
