...
void main() {
  group('KpParkingTypeRemoteDataSource', () {
    late KpParkingTypeRemoteDataSource dataSource;
    ...
    setUp(() {
      ...
      dataSource = KpParkingTypeRemoteDataSourceImpl(apiClient);
    });

    group('getKpParkingTypes', () {
      test('should fetch parking types from API and map to models', () async {
        // Act
        final result = await dataSource.getKpParkingTypes();
        ...
        result.fold(
          ...
          (models) {
            ...
            expect(models, isA<List<KpParkingTypeModel>>());
            expect(models.isNotEmpty, isTrue);

            // Log the actual data for verification
            AppLogger.d('Fetched ${models.length} parking types:\n${models.join('\n')}');
          },
        );
      });
    });
