...
void main() {
  group('KpDiscountCategoryRemoteDataSource', () {
    late KpDiscountCategoryRemoteDataSource dataSource;
    ...
    setUp(() {
      ...
      dataSource = KpDiscountCategoryRemoteDataSourceImpl(apiClient);
    });

    group('getKpDiscountCategories', () {
      test('should fetch discount categories from API and map to models', () async {
        // Act
        final result = await dataSource.getKpDiscountCategories();
        ...
        result.fold(
          ...
          (models) {
            ...
            expect(models, isA<List<KpDiscountCategoryModel>>());
            expect(models.isNotEmpty, isTrue);

            // Log the actual data for verification
            AppLogger.d('Fetched ${models.length} discount categories:\n${models.join('\n')}');
          },
        );
      });
    });
